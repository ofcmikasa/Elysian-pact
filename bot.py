...
def get_user(uid: str) -> dict:
    ...
    if uid not in users_db:
        users_db[uid] = {"ink": 0, "total_hours": 0.0, "streak": 0, "last_study_date": None, "last_daily": None}
    return users_db[uid]
...
def clean_nickname(name: str) -> str:
    cleaned = "".join(c for c in name if not unicodedata.category(c).startswith("C")).strip()
...
async def on_message(message):
    ...
    images = sum(1 for a in message.attachments if any(a.filename.lower().endswith(x) for x in [".png",".jpg",".jpeg",".gif",".webp"]))
    if images >= 5:
        try:
            await message.delete()
            await message.channel.send(f"{message.author.mention} The Gallery is full. Please wait a moment.", delete_after=8)
        except Exception:
            pass
        return
    ...
    if len(message_tracker[uid]) >= 10:
        ...
        if now - last_warned > 600:
            ...
            message_tracker[uid] = []
            try:
                await message.author.send("📚 *Scholar, your books are waiting.* You've been very active in chat. Shall I mute this channel for you so you can focus?")
            except Exception:
                pass
...
async def on_message_delete(message):
    ...
    if message.mentions:
        ...
        em.add_field(name="Pinger",         value=f"{message.author.mention} (`{message.author.id}`)", inline=False)
        em.add_field(name="Pinged",          value=pinged, inline=False)
        em.add_field(name="Channel",         value=message.channel.mention, inline=False)
        em.add_field(name="Deleted Message", value=message.content or "*(no text)*", inline=False)
        ...
        em.add_field(name="Scholar",  value=f"{message.author.mention} (`{message.author.id}`)", inline=False)
        em.add_field(name="Location", value=message.channel.mention, inline=False)
        em.add_field(name="Content",  value=message.content or "*(attachment only)*", inline=False)
    ...
    for att in message.attachments:
        if any(att.filename.lower().endswith(x) for x in [".png",".jpg",".jpeg",".gif",".webp"]):
            em = discord.Embed(title="📷 Vanished Media Recovered", description=f"Deleted by {message.author.mention} in {message.channel.mention}", color=0xffa500)
    ...
    if tmpl:
        ...
        if ch:
            try:
                ...
                color = 0x7B5EA7
            em = discord.Embed(title=fill_vars(tmpl.get("title","Welcome!"),member), description=fill_vars(tmpl.get("description",""),member), color=color)
            em.set_thumbnail(url=member.display_avatar.url)
            if tmpl.get("image"):
...
async def on_member_remove(member):
    ...
    try:
        ...
        color = 0xed4245
    em = discord.Embed(title=fill_vars(tmpl.get("title","Farewell!"),member), description=fill_vars(tmpl.get("description",""),member), color=color)
    em.set_thumbnail(url=member.display_avatar.url)
    if tmpl.get("footer"):
        em.set_footer(text=fill_vars(tmpl["footer"],member))
    await ch.send(embed=em)
...
async def on_voice_state_update(member, before, after):
    ...
    joined = after.channel and after.channel.id in svcs and (not before.channel or before.channel.id not in svcs)
    left   = before.channel and before.channel.id in svcs and (not after.channel or after.channel.id not in svcs)
...
class PurgeUserModal(discord.ui.Modal, title="🗑️ Cleanse User Messages"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            return await interaction.response.send_message("Please enter a valid number.", ephemeral=True)
        await interaction.response.defer(ephemeral=True)
        deleted = await interaction.channel.purge(limit=n, check=lambda m: m.author.id == self.target.id)
...
class KickModal(discord.ui.Modal, title="👣 Remove Scholar"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        reason = self.reason.value.strip() or "No reason provided"
        try:
            await self.target.send(f"👣 Removed from **{interaction.guild.name}**. Reason: **{reason}**")
        except Exception:
            pass
...
class BanModal(discord.ui.Modal, title="🔒 Exile Scholar"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        reason = self.reason.value.strip() or "No reason provided"
        try:
            await self.target.send(f"🔒 Exiled from **{interaction.guild.name}**. Reason: **{reason}**")
        except Exception:
            pass
...
class DeepWorkModal(discord.ui.Modal, title="🧠 Deep Work Mode"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if uid in deepwork_tasks:
            return await interaction.response.send_message("You're already in Deep Work mode.", ephemeral=True)
        try:
            m = int(self.minutes.value.strip())
        except ValueError:
            return await interaction.response.send_message("Please enter a valid number.", ephemeral=True)
...
async def lock(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(embed=discord.Embed(description="🔒 *The gates are sealed. This chamber demands silence.*", color=0x2f3136))
...
async def profile(interaction: discord.Interaction, user: discord.Member = None):
    ...
    s      = data.get("streak", 0)
    em = discord.Embed(title=f"✦ {target.display_name} {streak_icon(s)}", color=TIER_COLORS.get(tier, 0x7B5EA7))
    ...
    if uid in voice_join_times:
        em.add_field(name="🎙️ In Study VC", value=f"{(time.time()-voice_join_times[uid])/60:.0f}m active", inline=False)
    em.set_footer(text="Elysian Prestige System")
    await interaction.response.send_message(embed=em)
...
async def daily(interaction: discord.Interaction):
    ...
    if data.get("last_daily") == today:
        return await interaction.response.send_message("✨ Already collected today's blessing. Return tomorrow.", ephemeral=True)
...
async def endfocus(interaction: discord.Interaction):
    ...
    if uid not in sessions_db:
        return await interaction.response.send_message("No active focus session. Use `/focus` to start one.", ephemeral=True)
    ...
    em.add_field(name="💧 Ink Earned", value=f"{int(elapsed_h*10)}{bonus}",   inline=True)
    ...
    try:
        ...
        if study_role:
            try:
                ...
                pass
        try:
            await member.send("📚 **Break over!** Back to work, scholar. Run `/pomodoro` again for another round.")
        except Exception:
            pass
